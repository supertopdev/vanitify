...
class AccountReportsController extends Controller
{
    ...
    public function balanceSheet()
    {
        ...
        if (request()->ajax()) {
            ...
            $purchase_details = $this->transactionUtil->getPurchaseTotals($business_id, 
                null, $end_date);
            $sell_details = $this->transactionUtil->getSellTotals($business_id, null, 
                $end_date);
    ...
    private function getAccountBalance($business_id, $end_date, $account_type = 'others'){
        ...
        $query = Account::leftjoin('account_transactions as AT', 
                                'AT.account_id', '=', 'accounts.id')
                                // ->NotClosed()
    ...
    public function paymentAccountReport()
    {
        ...
        if(request()->ajax()){
            $query = TransactionPayment::leftjoin('transactions as T', 
