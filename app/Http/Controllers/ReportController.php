...
class ReportController extends Controller
{
            ...
            return Datatables::of($query)
                ->editColumn('product_name', function ($row) {
                 ...
                 ->editColumn('quantity_adjusted', function ($row) {
                    return '<span class="display_currency quantity_adjusted" data-currency_symbol=false data-orig-value="' . (float)$row->quantity_adjusted . '" data-unit="' . $row->unit . '" >' . (float) $row->quantity_adjusted . '</span> ' . $row->unit;
                 })
    ...
    public function adjustProductStock()
    {
        ...
        if(!empty(request()->input('variation_id'))
            ...
            $vld = VariationLocationDetails::leftjoin('business_locations as bl', 
                'bl.id', '=', 'variation_location_details.location_id')
