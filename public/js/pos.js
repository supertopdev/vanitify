$(document).ready( function(){
	...
	.autocomplete( "instance" )._renderItem = function( ul, item ) {
		if(item.enable_stock == 1 && item.qty_available <= 0){
        	...
        	if(item.enable_stock == 1) {
        		var qty_available = __currency_trans_from_en(item.qty_available, false);
	...
	$('table#pos_table tbody').on('change', 'select.lot_number', function(){
		...
		if($(this).val()){
			...
			if(sub_unit_length > 0) {
				...
				max_err_msg = __translate('lot_max_qty_error', {'max_val': lot_qty_formated, 'unit_name': unit_name});
			}
			...
			if(sub_unit_length > 0) {
				...
				default_err_msg = __translate('pos_max_qty_error', {'max_val': lot_qty_formated, 'unit_name': unit_name});
			}
	...
	$("table#pos_table").on('change', 'select.sub_unit', function(){
        ...
        if(base_max_avlbl) {
        	...
        	var max_err_msg = __translate(error_msg_line, {'max_val': formated_max_avlbl, 'unit_name': unit_name});
